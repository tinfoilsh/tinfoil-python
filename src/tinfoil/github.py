...
def _ensure_github_cache_dir() -> None:
    """Create the GitHub cache directory if it doesn't exist. Silently ignores failures."""
    try:
...
def fetch_latest_digest(repo: str) -> str:
    ...
    if not isinstance(response_data, dict) or "tag_name" not in response_data:
        raise ValueError(f"Unexpected release API response for {repo}: missing 'tag_name'")
