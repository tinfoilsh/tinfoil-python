...
from .abi_sev import (Report, ReportSigner, TCBParts)
...
from pyasn1.type.univ import Integer as Asn1Integer
from pyasn1.type.char import IA5String as Asn1IA5String
import warnings
...
# VCEK cache directory (created lazily on first use)
_VCEK_CACHE_DIR = platformdirs.user_cache_dir("tinfoil", "tinfoil")


def _ensure_vcek_cache_dir() -> None:
    """Create the VCEK cache directory if it doesn't exist. Silently ignores failures."""
    try:
...
class CertificateChain:
    ...
    def _validate_vcek_format(self):
        ...
        product_name = _decode_der_ia5string(extensions[SnpOid.PRODUCT_NAME_1])
        if product_name != "Genoa":
            raise ValueError(f"unexpected PRODUCT_NAME_1 in VCEK certificate: {product_name}")
...
def _decode_der_integer(der_bytes: bytes) -> int:
    """Decode a DER-encoded INTEGER, rejecting non-INTEGER types and trailing bytes."""
    try:
        result, remainder = der_decoder.decode(der_bytes, asn1Spec=Asn1Integer())
    except Exception as e:
        raise ValueError(f"Failed to decode DER integer: {e}") from e
    if remainder:
        raise ValueError(
            f"Unexpected trailing bytes after DER integer: {len(remainder)} bytes"
        )
    return int(result)

def _decode_der_ia5string(der_bytes: bytes) -> str:
    """Decode a DER-encoded IA5String, rejecting non-IA5String types and trailing bytes."""
    try:
        result, remainder = der_decoder.decode(der_bytes, asn1Spec=Asn1IA5String())
    except Exception as e:
        raise ValueError(f"Failed to decode DER IA5String: {e}") from e
    if remainder:
        raise ValueError(
            f"Unexpected trailing bytes after DER IA5String: {len(remainder)} bytes"
        )
    return str(result)
...
def _VCEKCertURL(productName: str, chip_id: bytes, reported_tcb: int) -> str:
    ...
    path = f"{base_url}/{urllib.parse.quote(productName, safe='')}/{chip_id_hex}"
    query = urllib.parse.urlencode({
        "blSPL": parts.bl_spl,
        "teeSPL": parts.tee_spl,
        "snpSPL": parts.snp_spl,
        "ucodeSPL": parts.ucode_spl,
    })
    return f"{path}?{query}"
